...
/// # Errors
///
/// This function will return an error if the request to the GitHub API fails or if the response
/// cannot be deserialized into an `UpstreamVersion`.
///
/// # Example
///
/// ```rust,no_run
...
pub async fn get_upstream_stable(client: &Client) -> Result<UpstreamVersion> {
    let response =
        make_github_request(client, "https://api.github.com/repos/neovim/neovim/releases/latest").await?;
