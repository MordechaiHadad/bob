...
use crate::github_requests::{deserialize_response, get_upstream_stable, make_github_request};
...
pub async fn start(config: Config, client: Client) -> Result<()> {
    ...
    let response =
        make_github_request(&client, "https://api.github.com/repos/neovim/neovim/tags?per_page=100").await?;
